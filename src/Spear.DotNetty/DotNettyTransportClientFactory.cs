...
namespace Spear.DotNetty
{
    ...
    public class DotNettyTransportClientFactory : ITransportClientFactory, IDisposable
    {
        ...
        public DotNettyTransportClientFactory(IMessageCoderFactory codecFactory)
            : this(codecFactory, null)
        {
        }

        public DotNettyTransportClientFactory(IMessageCoderFactory codecFactory, IServiceExecutor serviceExecutor)
        {
            _transportMessageEncoder = codecFactory.GetEncoder();
            var transportMessageDecoder = codecFactory.GetDecoder();
            _logger = LogManager.Logger<DotNettyTransportClientFactory>();
                    ...
                    , k => new Lazy<ITransportClient>(() =>
                        {
                            ...
                            var messageSender = new DotNettyClientMessageSender(_transportMessageEncoder, channel);
